...
export default function PresetEditor({ classId, readOnly = false }: Props) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 md:px-10 py-6 max-w-3xl">
      ...
      {!readOnly && <button onClick={addColumn} className="text-xs text-muted hover:text-ink mb-8">+ Add column</button>}
      ...
      <div className="space-y-2 mb-3">
          ...
          <div key={i} className="rounded-xl border border-border/70 bg-card-bg p-3 space-y-2">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                disabled={readOnly}
                className="px-2 py-1.5 text-xs rounded-lg border border-border bg-column-bg text-ink outline-none disabled:opacity-60"
              >
              ...
              <select
                ...
                disabled={readOnly}
                className="px-2 py-1.5 text-xs rounded-lg border border-border bg-column-bg text-ink outline-none capitalize disabled:opacity-60"
              >
        ...
        {readOnly && tasks.length === 0 && <p className="text-[11px] text-muted">No seed tasks.</p>}
      </div>
      {!readOnly && <button onClick={addTask} className="text-xs text-muted hover:text-ink mb-8">+ Add seed task</button>}
