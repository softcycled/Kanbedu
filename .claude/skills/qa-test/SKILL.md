...
reuse it. Known working local account: `hominghao1710@gmail.com` / `demo1234` (real
account, also owns the demo seed data — see `prisma/seed.ts`).
...
(async () => {
  ...
  await page.fill('input[placeholder="you@example.com"]', 'hominghao1710@gmail.com');
  await page.fill('input[placeholder="Your password"]', 'demo1234');
...
- Delete the scratch script/screenshot directory (`.qa/` or similar) — don't leave
  it in the repo; check `git status --short` afterward to confirm.
- Stop the dev server you started (`TaskStop` on its task ID) unless the user is
  actively using it.
- If you created throwaway test data (boards, classes, users) in the local DB and
  it's not part of an agreed demo dataset, consider whether to clean it up too —
  but don't run raw cascading deletes by hand (`prisma.class.deleteMany` etc. can
  partially fail mid-cascade and leave orphaned rows that look like real bugs on the
  next session — this happened once, see project_changelog.md). Prefer going
  through the app's own delete UI/API if cleanup is needed.
